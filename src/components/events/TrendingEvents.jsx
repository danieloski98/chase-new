...
const TrendingEvents = () => {
  ...
  const { results, isLoading, lastChildRef, refetch } = useInfinteScroller({url:`/events/events`, pageNumber:page, setPageNumber:setPage})
              ...
              if (results.length === i + 1) {
                return(
                  ...
                  <div className=" w-full h-full flex flex-col pb-4 " >
                    <div
                        role='button'
                        onClick={() => navigate(`/events/${event.id}`)} className=" w-full flex justify-between items-center gap-2 py-2 border-b " > 
                      <p className=" font-bold text-lg " >{event.eventName?.length >= 10 ? event.eventName.slice(0, 10)+"..." : event.eventName}</p>
