      ...
      <li key={event.id} className="border-b w-full p-2 px-4 ">
        <div className="flex my-4 items-center w-full justify-between">
          ...
          <div className=' w-full' >
            <div className="flex flex-col gap-4 flex-1 w-full justify-end pl-4">
              <div onClick={() => navigate(`/events/${event.id}`)}  className="border-b text-[18px] w-full flex items-center justify-between font-bold cursor-pointer">
                <span className=' ' >{event.eventName?.length >= 8 ? event.eventName.slice(0, 8)+"..." : event.eventName}{" "}</span>

                <span className=" ml-auto w-20 lg:mr-8  text-left md:pl-4">{event?.currency === "USD" ? "$" : "₦"}{event?.maxPrice}</span>
                {/* <span className="pl-1 md:pl-4">{event.maxPrice}</span> */}
              </div>
              <span className="flex gap-2 text-xs md:text-sm text-[#2E2B2B] text-opacity-[67%]">
                <CalendarIcon /> 
  ...
  return (
    <div className="px-4 h-auto overflow-auto pb-96">
     {/* {!isLoading && ( */}
       ...
       {dataInfo?.map((post: any, i: number) => {
        if (results.length === i + 1) {
          return (
              <SearchEventCard {...post} ref={lastChildRef} />
            )
        } else {
          return (
