...
function EventDraft() {
    ...
    const { results, isLoading, ref, refetch, isRefetching } = InfiniteScrollerComponent({url:'/events/drafts?createdBy='+userId, limit:10, filter: "id", newdata: false})
    ...
    const EventInfo =(event)=> {
        return( 
            ...
            >
                ...
                <div className=' w-fit ' >
                    <div  role='button' onClick={()=> editHandler(event)}  className="w-44 h-40 object-cover rounded-b-[32px] rounded-tl-[32px]">
                        {event?.currentPicUrl ? (
                            <img
                ...
                <div className="flex w-full flex-col gap-2">
                    <h2 role='button' onClick={()=> editHandler(event)} className="text-lg font-medium text-center md:text-left">
                    {event?.eventName}
                    </h2>
                    {event?.startTime && (
                        <div className="flex">
