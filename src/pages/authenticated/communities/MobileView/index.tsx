...
function MobileViewChat({ query }: IProps) {
    ...
    return (
        <HStack width='100%' height='100%' gap={0}>
            ...
            {activeCommunity === null && (
              <VStack flex='1' bg='white' height='100%' shadow='md' zIndex={3}>
              {/* HEADER */}
              ...
              <VStack flex={1} overflow='auto' width='100%' px='20px' height='100%' paddingBottom='100px'>
                  {!query.isLoading && communities.map((community, i) => (
                      <CommunityCard community={community} key={i} setSelected={(data: ICommunity) => setActiveCommuntiy(data)} smallScreen />
                  ))}
              </VStack>
          </VStack>
            )}
            ...
            {/* CHAT AREA */}
            { activeCommunity && (
              <VStack flex='1' height='100%' width='100%' overflow='hidden' spacing={0} zIndex={1} className='bg-[url("/src/assets/images/chat-bg.png")]'>
              {activeCommunity === null && (
              ...
              {activeCommunity !== null && (
                  <VStack flex='1' width='100%' height='100%' spacing={0}>
                      ...
                      <Flex flexDirection='column' flex='0.8' overflow={'auto'} width='100%' height='100%' >
                          {events.length > 0 && (
                              <Box width='100%' height='120px'>
                                <EventsPanel communityId={activeCommunity.id} events={events} toggleEvents={toggleEvents} />
                              </Box>
                          )}
                          <HStack width='100%' height='100px' justifyContent='center' paddingX='50px' marginTop='10px'>
                              {events.length < 1 && (
